...
namespace CaptainNerfs
{
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        // public static ConfigEntry<string> BalancePoint { get; set; }
        ...
        private void Awake()
        {
            ...
            // EnableMod = Config.Bind(new ConfigDefinition("CaptainNerfs", "EnableMod"), true, new ConfigDescription("Enables the mod. If false, the mod will not work then next time you load the game."));
            // EnableDebugging = Config.Bind(new ConfigDefinition("CaptainNerfs", "EnableDebugging"), true, new ConfigDescription("Enables the debugging"));
            AcceptableValueList<string> balancePoints = new AcceptableValueList<string>(["Balanced", "Early", "OP", "All"]);
            // BalancePoint = Config.Bind(new ConfigDefinition("CaptainNerfs", "RandomFish"), "Balanced", new ConfigDescription("Determines how balanced the pets are. Balanced is similar to other pets, Early is balanced around players with no perks/early play, OP is OP.", acceptableValues: balancePoints));


            // DevMode = Config.Bind(new ConfigDefinition("CaptainNerfs", "DevMode"), false, new ConfigDescription("Enables all of the things for testing."));

            // string contentFolderName = BalancePoint.Value switch
            // {
            //     "Balanced" => "CaptainNerfsBalanced",
            //     "Early" => "CaptainNerfsEarly",
            //     "OP" => "CaptainNerfsOP",
            //     "All" => "CaptainNerfsAll",
            //     _ => "CaptainNerfsBalanced"
            // };
            string contentFolderName = "CaptainNerfs";
            // Register with Obeliskial Essentials, delete this if you don't need it.
            RegisterMod(
                _name: PluginInfo.PLUGIN_NAME,
                _author: "binbin",
                _description: "Captain Nerfs",
                _version: PluginInfo.PLUGIN_VERSION,
                _date: ModDate,
                _link: @"https://github.com/binbinmods/CaptainNerfs",
                _contentFolder: contentFolderName
            );

            // apply patches, this functionally runs all the code for Harmony, running your mod
            if (true) { harmony.PatchAll(); }
        }
